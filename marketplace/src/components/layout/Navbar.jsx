...
export default function Navbar() {
  ...
  // Cierra dropdown desktop al click afuera
  useEffect(() => {
    ...
    <>
      ...
      <header className="navbar">
        <div className="navbar-inner container">
          ...
          {/* ── Mobile: carrito + hamburguesa (siempre visible) ── */}
      ...
      {/* ── Drawer móvil (renderizado solo cuando está abierto) ── */}
      {mobileOpen && (
        <>
          ...
          <nav className="mobile-drawer">
            ...
            {state.isAuthenticated ? (
              <div className="drawer-user">
                ...
                <div>
                  ...
                  <p className="drawer-user-email text-small text-muted">{state.user?.email}</p>
                </div>
              </div>
            ) : (
            ...
            <NavLink to="/carrito" className={({ isActive }) => `drawer-link${isActive ? ' active' : ''}`} onClick={closeAll}>
              <span className="drawer-link-inner">
                Carrito
                {cartCount > 0 && <span className="drawer-cart-badge">{cartCount > 99 ? '99+' : cartCount}</span>}
              </span>
            </NavLink>
